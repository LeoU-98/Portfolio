...
function SkillButton({ imageSource, alt, text, setSkillsData, isActive }) {
  ...
  return (
    <li onClick={() => setSkillsData(corrispondingData)}>
      <div
        className={`${isActive ? 'bg-gradient-to-br' : ''} relative mx-auto h-20 w-36 cursor-pointer select-none rounded-xl border-[1px] border-violet-700 from-violet-600 to-violet-900 duration-300 hover:bg-gradient-to-r active:translate-y-4`}
      >
        <img
          src={imageSource}
          alt={alt}
          className={`absolute -left-4 -top-4 size-10 ${isActive ? 'animate-pulse' : ''} rounded-lg border-2 border-violet-700 bg-[#151515] p-1`}
